...
export async function POST(request: NextRequest) {
  console.log('🚀 API /api/notify appelée')
  ...
  try {
    ...
    if (!email || !downloadTime) {
      ...
      return NextResponse.json(
        { success: false, message: 'Email et downloadTime requis' },
        { status: 400 }
      )
    }
    ...
    if (process.env.RESEND_API_KEY) {
      try {
        ...
        const { data, error } = await resend.emails.send({
          ...
          to: process.env.RESEND_ACCOUNT_EMAIL || 'faycal.zouaoui@wedreamteam.com',
...
export async function GET() {
  console.log('🔍 Test GET /api/notify')
  return NextResponse.json({ 
    message: 'API notify fonctionne',
    timestamp: new Date().toISOString(),
    config: {
      hasResendKey: !!process.env.RESEND_API_KEY,
      notificationEmail: process.env.RESEND_ACCOUNT_EMAIL
    }
  })
}
